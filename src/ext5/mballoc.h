...
#define mb_debug(sb, fmt, ...)						\
	pr_debug("[%s/%d] EXT5-fs (%s): (%s, %d): %s: " fmt,		\
 ...
 * We can tune the same via /proc/fs/ext5/<partition>/stream_req
 */
...
struct ext5_allocation_context {
	 ...
	 */
	ext5_grpblk_t	ac_orig_goal_len;

	__u32 ac_flags;		/* allocation hints */
	__u32 ac_groups_linear_remaining;
	__u16 ac_groups_scanned;
	__u16 ac_found;
	__u16 ac_cX_found[EXT5_MB_NUM_CRS];
	__u16 ac_tail;
	__u16 ac_buddy;
	__u8 ac_status;
	__u8 ac_criteria;
...
static inline loff_t extent_logical_end(struct ext5_sb_info *sbi,
					struct ext5_free_extent *fex)
{
	/* Use loff_t to avoid end exceeding ext5_lblk_t max. */
	return (loff_t)fex->fe_logical + EXT5_C2B(sbi, fex->fe_len);
}

static inline loff_t pa_logical_end(struct ext5_sb_info *sbi,
				    struct ext5_prealloc_space *pa)
{
	/* Use loff_t to avoid end exceeding ext5_lblk_t max. */
	return (loff_t)pa->pa_lstart + EXT5_C2B(sbi, pa->pa_len);
}

typedef int (*ext5_mballoc_query_range_fn)(
