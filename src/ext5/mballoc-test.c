...
/*
 * KUnit test of ext5 multiblocks allocation.
 */
...
struct mbt_ext5_super_block {
	struct ext5_super_block es;
	struct ext5_sb_info sbi;
	struct mbt_ctx mbt_ctx;
};

#define MBT_SB(_sb) (container_of((_sb)->s_fs_info, struct mbt_ext5_super_block, sbi))
...
static struct inode *mbt_alloc_inode(struct super_block *sb)
{
	...
	ei = kmalloc(sizeof(struct ext5_inode_info), GFP_KERNEL);
	if (!ei)
		return NULL;
...
static int mbt_mb_init(struct super_block *sb)
{
	...
	/* needed by ext5_mb_init->bdev_nonrot(sb->s_bdev) */
	...
	/*
	 * needed by ext5_mb_init->ext5_mb_init_backend-> sbi->s_buddy_cache =
	 * new_inode(sb);
	 */
	...
	block = ext5_count_free_clusters(sb);
	ret = percpu_counter_init(&EXT5_SB(sb)->s_freeclusters_counter, block,
				  GFP_KERNEL);
	if (ret != 0)
		goto err_mb_release;

	ret = percpu_counter_init(&EXT5_SB(sb)->s_dirtyclusters_counter, 0,
				  GFP_KERNEL);
	if (ret != 0)
...
err_freeclusters:
	percpu_counter_destroy(&EXT5_SB(sb)->s_freeclusters_counter);
err_mb_release:
	ext5_mb_release(sb);
err_out:
...
{
	percpu_counter_destroy(&EXT5_SB(sb)->s_dirtyclusters_counter);
	percpu_counter_destroy(&EXT5_SB(sb)->s_freeclusters_counter);
	ext5_mb_release(sb);
	kfree(sb->s_bdev->bd_queue);
	kfree(sb->s_bdev);
...
static struct super_block *mbt_ext5_alloc_super_block(void)
{
	struct mbt_ext5_super_block *fsb;
...
static void mbt_ext5_free_super_block(struct super_block *sb)
{
	struct mbt_ext5_super_block *fsb = MBT_SB(sb);
...
struct mbt_ext5_block_layout {
	unsigned char blocksize_bits;
	unsigned int cluster_bits;
	uint32_t blocks_per_group;
	ext5_group_t group_count;
	uint16_t desc_size;
};

static void mbt_init_sb_layout(struct super_block *sb,
			       struct mbt_ext5_block_layout *layout)
{
	struct ext5_sb_info *sbi = EXT5_SB(sb);
			    ...
			    struct mbt_grp_ctx *grp_ctx)
{
	...
	grp_ctx->bitmap_bh.b_data = kzalloc(EXT5_BLOCK_SIZE(sb), GFP_KERNEL);
	if (grp_ctx->bitmap_bh.b_data == NULL)
		return -ENOMEM;
	mb_set_bits(grp_ctx->bitmap_bh.b_data, max, sb->s_blocksize * 8 - max);
	ext5_free_group_clusters_set(sb, &grp_ctx->desc, max);
...
static void mbt_ctx_mark_used(struct super_block *sb, ext5_group_t group,
...
static void *mbt_ctx_bitmap(struct super_block *sb, ext5_group_t group)
{
	struct mbt_grp_ctx *grp_ctx = MBT_GRP_CTX(sb, group);
...
static int mbt_ctx_init(struct super_block *sb)
{
	 ...
	 * block which will fail ext5_sb_block_valid check.
	 */
	mb_set_bits(ctx->grp_ctx[0].bitmap_bh.b_data, 0, 1);
	ext5_free_group_clusters_set(sb, &ctx->grp_ctx[0].desc,
				     EXT5_CLUSTERS_PER_GROUP(sb) - 1);
...
ext5_read_block_bitmap_nowait_stub(struct super_block *sb, ext5_group_t block_group,
				   bool ignore_locked)
{
	struct mbt_grp_ctx *grp_ctx = MBT_GRP_CTX(sb, block_group);

	/* paired with brelse from caller of ext5_read_block_bitmap_nowait */
	get_bh(&grp_ctx->bitmap_bh);
	return &grp_ctx->bitmap_bh;
}

static int ext5_wait_block_bitmap_stub(struct super_block *sb,
				       ext5_group_t block_group,
				       struct buffer_head *bh)
{
	/*
	 * real ext5_wait_block_bitmap will set these flags and
	 * functions like ext5_mb_init_cache will verify the flags.
	 */
...
static struct ext5_group_desc *
ext5_get_group_desc_stub(struct super_block *sb, ext5_group_t block_group,
			 struct buffer_head **bh)
{
	struct mbt_grp_ctx *grp_ctx = MBT_GRP_CTX(sb, block_group);
...
static int
ext5_mb_mark_context_stub(handle_t *handle, struct super_block *sb, bool state,
			  ext5_group_t group, ext5_grpblk_t blkoff,
			  ext5_grpblk_t len, int flags,
			  ext5_grpblk_t *ret_changed)
{
	struct mbt_grp_ctx *grp_ctx = MBT_GRP_CTX(sb, group);
...
{
	struct mbt_ext5_block_layout *layout =
		(struct mbt_ext5_block_layout *)(test->param_value);
	struct super_block *sb;
	int ret;

	sb = mbt_ext5_alloc_super_block();
	if (sb == NULL)
		return -ENOMEM;
	...
	kunit_activate_static_stub(test,
				   ...
				   ext5_read_block_bitmap_nowait_stub);
	kunit_activate_static_stub(test,
				   ext5_wait_block_bitmap,
				   ext5_wait_block_bitmap_stub);
	kunit_activate_static_stub(test,
				   ext5_get_group_desc,
				   ext5_get_group_desc_stub);
	kunit_activate_static_stub(test,
				   ext5_mb_mark_context,
				   ext5_mb_mark_context_stub);

	/* stub function will be called in mbt_mb_init->ext5_mb_init */
	if (mbt_mb_init(sb) != 0) {
		mbt_ctx_release(sb);
		mbt_ext5_free_super_block(sb);
		return -ENOMEM;
...
static void test_new_blocks_simple(struct kunit *test)
{
	...
	ext5_group_t i, goal_group = TEST_GOAL_GROUP;
	int err = 0;
	ext5_fsblk_t found;
	struct ext5_sb_info *sbi = EXT5_SB(sb);
	...
	/* get block at goal */
	ar.goal = ext5_group_first_block_no(sb, goal_group);
	found = ext5_mb_new_blocks_simple(&ar, &err);
	...
	ar.goal = ext5_group_first_block_no(sb, goal_group);
	found = ext5_mb_new_blocks_simple(&ar, &err);
	KUNIT_ASSERT_EQ_MSG(test, ar.goal + EXT5_C2B(sbi, 1), found,
		"failed to alloc block after goal in goal group, expected %llu found %llu",
		ar.goal + 1, found);

	/* get block after goal group */
	mbt_ctx_mark_used(sb, goal_group, 0, EXT5_CLUSTERS_PER_GROUP(sb));
	ar.goal = ext5_group_first_block_no(sb, goal_group);
	found = ext5_mb_new_blocks_simple(&ar, &err);
	KUNIT_ASSERT_EQ_MSG(test,
		ext5_group_first_block_no(sb, goal_group + 1), found,
		"failed to alloc block after goal group, expected %llu found %llu",
		ext5_group_first_block_no(sb, goal_group + 1), found);

	/* get block before goal group */
	for (i = goal_group; i < ext5_get_groups_count(sb); i++)
		mbt_ctx_mark_used(sb, i, 0, EXT5_CLUSTERS_PER_GROUP(sb));
	ar.goal = ext5_group_first_block_no(sb, goal_group);
	found = ext5_mb_new_blocks_simple(&ar, &err);
	KUNIT_ASSERT_EQ_MSG(test,
		ext5_group_first_block_no(sb, 0) + EXT5_C2B(sbi, 1), found,
		"failed to alloc block before goal group, expected %llu found %llu",
		ext5_group_first_block_no(sb, 0 + EXT5_C2B(sbi, 1)), found);

	/* no block available, fail to allocate block */
	for (i = 0; i < ext5_get_groups_count(sb); i++)
		mbt_ctx_mark_used(sb, i, 0, EXT5_CLUSTERS_PER_GROUP(sb));
	ar.goal = ext5_group_first_block_no(sb, goal_group);
	found = ext5_mb_new_blocks_simple(&ar, &err);
...
mbt_generate_test_ranges(struct super_block *sb, struct test_range *ranges,
			 int count)
{
	ext5_grpblk_t start, len, max;
	int i;

	max = EXT5_CLUSTERS_PER_GROUP(sb) / count;
	for (i = 0; i < count; i++) {
...
validate_free_blocks_simple(struct kunit *test, struct super_block *sb,
			    ext5_group_t goal_group, ext5_grpblk_t start,
			    ext5_grpblk_t len)
{
	void *bitmap;
	ext5_grpblk_t bit, max = EXT5_CLUSTERS_PER_GROUP(sb);
	ext5_group_t i;
...
static void
test_free_blocks_simple_range(struct kunit *test, ext5_group_t goal_group,
			      ext5_grpblk_t start, ext5_grpblk_t len)
{
	struct super_block *sb = (struct super_block *)test->priv;
	struct ext5_sb_info *sbi = EXT5_SB(sb);
	struct inode *inode;
	...
	block = ext5_group_first_block_no(sb, goal_group) +
		EXT5_C2B(sbi, start);
	ext5_free_blocks_simple(inode, block, len);
...
test_mark_diskspace_used_range(struct kunit *test,
			       struct ext5_allocation_context *ac,
			       ext5_grpblk_t start,
			       ext5_grpblk_t len)
{
	struct super_block *sb = (struct super_block *)test->priv;
	...
	/* ext5_mb_mark_diskspace_used will BUG if len is 0 */
	if (len == 0)
		return;
	...
	ret = ext5_mb_mark_diskspace_used(ac, NULL, 0);
	KUNIT_ASSERT_EQ(test, ret, 0);

	max = EXT5_CLUSTERS_PER_GROUP(sb);
...
static void mbt_generate_buddy(struct super_block *sb, void *buddy,
			       void *bitmap, struct ext5_group_info *grp)
{
	struct ext5_sb_info *sbi = EXT5_SB(sb);
	uint32_t order, off;
	void *bb, *bb_h;
	...
	memset(grp, 0, offsetof(struct ext5_group_info,
...
static void
mbt_validate_group_info(struct kunit *test, struct ext5_group_info *grp1,
			struct ext5_group_info *grp2)
{
	struct super_block *sb = (struct super_block *)test->priv;
	int i;
...
do_test_generate_buddy(struct kunit *test, struct super_block *sb, void *bitmap,
			   void *mbt_buddy, struct ext5_group_info *mbt_grp,
			   void *ext5_buddy, struct ext5_group_info *ext5_grp)
{
	int i;
	...
	for (i = 0; i < MB_NUM_ORDERS(sb); i++)
		ext5_grp->bb_counters[i] = 0;
	/* needed by validation in ext5_mb_generate_buddy */
	ext5_grp->bb_free = mbt_grp->bb_free;
	memset(ext5_buddy, 0xff, sb->s_blocksize);
	ext5_mb_generate_buddy(sb, ext5_buddy, bitmap, TEST_GOAL_GROUP,
			       ext5_grp);

	KUNIT_ASSERT_EQ(test, memcmp(mbt_buddy, ext5_buddy, sb->s_blocksize),
			0);
	mbt_validate_group_info(test, mbt_grp, ext5_grp);
}
...
static void test_mb_generate_buddy(struct kunit *test)
{
	...
	struct ext5_group_info *expected_grp, *generate_grp;
	struct test_range ranges[TEST_RANGE_COUNT];
	int i;
	...
	expected_grp = kunit_kzalloc(test, offsetof(struct ext5_group_info,
				bb_counters[MB_NUM_ORDERS(sb)]), GFP_KERNEL);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, expected_grp);
	generate_grp = ext5_get_group_info(sb, TEST_GOAL_GROUP);
...
static void
test_mb_mark_used_range(struct kunit *test, struct ext5_buddy *e4b,
			ext5_grpblk_t start, ext5_grpblk_t len, void *bitmap,
			void *buddy, struct ext5_group_info *grp)
{
	struct super_block *sb = (struct super_block *)test->priv;
	...
	ext5_lock_group(sb, TEST_GOAL_GROUP);
	mb_mark_used(e4b, &ex);
	ext5_unlock_group(sb, TEST_GOAL_GROUP);

	mb_set_bits(bitmap, start, len);
	/* bypass bb_free validatoin in ext5_mb_generate_buddy */
	grp->bb_free -= len;
	memset(buddy, 0xff, sb->s_blocksize);
	for (i = 0; i < MB_NUM_ORDERS(sb); i++)
		grp->bb_counters[i] = 0;
	ext5_mb_generate_buddy(sb, buddy, bitmap, 0, grp);
...
static void test_mb_mark_used(struct kunit *test)
{
	...
	grp = kunit_kzalloc(test, offsetof(struct ext5_group_info,
				bb_counters[MB_NUM_ORDERS(sb)]), GFP_KERNEL);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, grp);

	ret = ext5_mb_load_buddy(sb, TEST_GOAL_GROUP, &e4b);
	KUNIT_ASSERT_EQ(test, ret, 0);

	grp->bb_free = EXT5_CLUSTERS_PER_GROUP(sb);
	grp->bb_largest_free_order = -1;
...
static void
test_mb_free_blocks_range(struct kunit *test, struct ext5_buddy *e4b,
			  ext5_grpblk_t start, ext5_grpblk_t len, void *bitmap,
			  void *buddy, struct ext5_group_info *grp)
{
	struct super_block *sb = (struct super_block *)test->priv;
...
static void test_mb_free_blocks(struct kunit *test)
{
	...
	ex.fe_start = 0;
	ex.fe_len = EXT5_CLUSTERS_PER_GROUP(sb);
	ex.fe_group = TEST_GOAL_GROUP;
...
static const struct mbt_ext5_block_layout mbt_test_layouts[] = {
	{
		.blocksize_bits = 10,
		.cluster_bits = 3,
...
static void mbt_show_layout(const struct mbt_ext5_block_layout *layout,
			    char *desc)
{
