...
static __le32 ext5_mmp_csum(struct super_block *sb, struct mmp_struct *mmp)
{
	...
	csum = ext5_chksum(sbi->s_csum_seed, (char *)mmp, offset);

	return cpu_to_le32(csum);
}

static int ext5_mmp_csum_verify(struct super_block *sb, struct mmp_struct *mmp)
{
...
static void ext5_mmp_csum_set(struct super_block *sb, struct mmp_struct *mmp)
{
	if (!ext5_has_feature_metadata_csum(sb))
		return;
			  ...
			  ext5_fsblk_t mmp_block)
{
	...
	lock_buffer(*bh);
	ret = ext5_read_bh(*bh, REQ_META | REQ_PRIO, NULL, false);
	if (ret)
		goto warn_exit;
	...
	if (!ext5_mmp_csum_verify(sb, mmp)) {
		ret = -EFSBADCRC;
		goto warn_exit;
	}
	return 0;
warn_exit:
	brelse(*bh);
	*bh = NULL;
	ext5_warning(sb, "Error %d while reading MMP block %llu",
		     ret, mmp_block);
	return ret;
}
...
{
	__ext5_warning(sb, function, line, "%s", msg);
...
static int kmmpd(void *data)
{
	...
	struct buffer_head *bh = EXT5_SB(sb)->s_mmp_bh;
	struct mmp_struct *mmp;
	ext5_fsblk_t mmp_block;
	u32 seq = 0;
	...
	while (!kthread_should_stop() && !ext5_emergency_state(sb)) {
...
void ext5_stop_mmpd(struct ext5_sb_info *sbi)
{
	if (sbi->s_mmp_tsk) {
...
{
	return get_random_u32_below(EXT5_MMP_SEQ_MAX + 1);
}
...
skip:
	 ...
	 */
	EXT5_SB(sb)->s_mmp_tsk = kthread_run(kmmpd, sb, "kmmpd-%.*s",
