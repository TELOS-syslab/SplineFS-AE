 ...
 */
static unsigned ext5_num_overhead_clusters(struct super_block *sb,
					   ext5_group_t block_group,
					   struct ext5_group_desc *gdp)
{
	...
	ext5_fsblk_t end = start + EXT5_BLOCKS_PER_GROUP(sb) - 1;
	ext5_fsblk_t itbl_blk_start, itbl_blk_end;
	struct ext5_sb_info *sbi = EXT5_SB(sb);

	/* This is the number of clusters used by the superblock,
	 * block group descriptors, and reserved block group
	 * descriptor blocks */
	base_clusters = ext5_num_base_meta_clusters(sb, block_group);
	 ...
	 * accounted below.
	 */
	itbl_blk_start = ext5_inode_table(sb, gdp);
	itbl_blk_end = itbl_blk_start + sbi->s_itb_per_group - 1;
	if (itbl_blk_start <= end && itbl_blk_end >= start) {
		itbl_blk_start = max(itbl_blk_start, start);
		itbl_blk_end = min(itbl_blk_end, end);

		itbl_cluster_start = EXT5_B2C(sbi, itbl_blk_start - start);
		itbl_cluster_end = EXT5_B2C(sbi, itbl_blk_end - start);
...
static int ext5_init_block_bitmap(struct super_block *sb,
				   ...
				   struct ext5_group_desc *gdp)
{
	...
	if (ext5_block_in_group(sb, tmp, block_group))
					     ...
					     struct buffer_head **bh)
{
	...
	KUNIT_STATIC_STUB_REDIRECT(ext5_get_group_desc,
				   sb, block_group, bh);
...
static ext5_fsblk_t ext5_valid_block_bitmap_padding(struct super_block *sb,
						    ext5_group_t block_group,
						    struct buffer_head *bh)
{
	...
	next_zero_bit = ext5_find_next_zero_bit(bh->b_data, bitmap_size, offset);

	return (next_zero_bit < bitmap_size ? next_zero_bit : 0);
}

struct ext5_group_info *ext5_get_group_info(struct super_block *sb,
					    ext5_group_t group)
{
	struct ext5_group_info **grp_info;
	long indexv, indexh;

	if (unlikely(group >= EXT5_SB(sb)->s_groups_count))
		return NULL;
	indexv = group >> (EXT5_DESC_PER_BLOCK_BITS(sb));
	indexh = group & ((EXT5_DESC_PER_BLOCK(sb)) - 1);
	grp_info = sbi_array_rcu_deref(EXT5_SB(sb), s_group_info, indexv);
					    ...
					    struct buffer_head *bh)
{
	...
	ext5_grpblk_t offset;
	ext5_grpblk_t next_zero_bit;
	ext5_grpblk_t max_bit = EXT5_CLUSTERS_PER_GROUP(sb);
	ext5_fsblk_t blk;
	...
	if (offset < 0 || EXT5_B2C(sbi, offset) >= max_bit ||
	    !ext5_test_bit(EXT5_B2C(sbi, offset), bh->b_data))
		/* bad block bitmap */
		return blk;
	...
	offset = blk - group_first_block;
	if (offset < 0 || EXT5_B2C(sbi, offset) >= max_bit ||
	    EXT5_B2C(sbi, offset + sbi->s_itb_per_group - 1) >= max_bit)
		return blk;
	next_zero_bit = ext5_find_next_zero_bit(bh->b_data,
			EXT5_B2C(sbi, offset + sbi->s_itb_per_group - 1) + 1,
			EXT5_B2C(sbi, offset));
	if (next_zero_bit <
	    EXT5_B2C(sbi, offset + sbi->s_itb_per_group - 1) + 1)
...
static int ext5_validate_block_bitmap(struct super_block *sb,
				      struct ext5_group_desc *desc,
				      ext5_group_t block_group,
				      struct buffer_head *bh)
{
	...
	if (buffer_verified(bh))
		return 0;
	if (!grp || EXT5_MB_GRP_BBITMAP_CORRUPT(grp))
		return -EFSCORRUPTED;

	ext5_lock_group(sb, block_group);
	if (buffer_verified(bh))
		goto verified;
	if (unlikely(!ext5_block_bitmap_csum_verify(sb, desc, bh) ||
		     ext5_simulate_fail(sb, EXT5_SIM_BBITMAP_CRC))) {
	...
	blk = ext5_valid_block_bitmap_padding(sb, block_group, bh);
	if (unlikely(blk != 0)) {
		ext5_unlock_group(sb, block_group);
		ext5_error(sb, "bg %u: block %llu: padding at end of block bitmap is not set",
			   block_group, blk);
		ext5_mark_group_bitmap_corrupted(sb, block_group,
						 EXT5_GROUP_INFO_BBITMAP_CORRUPT);
		return -EFSCORRUPTED;
			      ...
			      bool ignore_locked)
{
	...
	KUNIT_STATIC_STUB_REDIRECT(ext5_read_block_bitmap_nowait,
	...
	if ((bitmap_blk <= le32_to_cpu(sbi->s_es->s_first_data_block)) ||
		...
		ext5_error(sb, "Invalid block bitmap block %llu in "
	...
	if (unlikely(!bh)) {
		ext5_warning(sb, "Cannot get buffer for block bitmap - "
	...
	if (ext5_has_group_desc_csum(sb) &&
		...
		if (block_group == 0) {
			ext5_unlock_group(sb, block_group);
			unlock_buffer(bh);
			ext5_error(sb, "Block bitmap for bg 0 marked "
				   "uninitialized");
			err = -EFSCORRUPTED;
			goto out;
		}
		err = ext5_init_block_bitmap(sb, bh, block_group, desc);
		if (err) {
			ext5_unlock_group(sb, block_group);
			unlock_buffer(bh);
			ext5_error(sb, "Failed to init block bitmap for group "
				   "%u: %d", block_group, err);
			goto out;
		}
	 ...
	 */
	set_buffer_new(bh);
	trace_ext5_read_block_bitmap_load(sb, block_group, ignore_locked);
	ext5_read_bh_nowait(bh, REQ_META | REQ_PRIO |
			    (ignore_locked ? REQ_RAHEAD : 0),
			    ext5_end_bitmap_read,
			    ext5_simulate_fail(sb, EXT5_SIM_BBITMAP_EIO));
	return bh;
verify:
	err = ext5_validate_block_bitmap(sb, desc, block_group, bh);
	if (err)
		goto out;
	return bh;
out:
			   ...
			   struct buffer_head *bh)
{
	...
	KUNIT_STATIC_STUB_REDIRECT(ext5_wait_block_bitmap,
				   sb, block_group, bh);
	...
	wait_on_buffer(bh);
	if (!buffer_uptodate(bh)) {
		ext5_error_err(sb, EIO, "Cannot read block bitmap - "
			       "block_group = %u, block_bitmap = %llu",
	...
	return ext5_validate_block_bitmap(sb, desc, block_group, bh);
}
...
ext5_read_block_bitmap(struct super_block *sb, ext5_group_t block_group)
{
	...
	bh = ext5_read_block_bitmap_nowait(sb, block_group, false);
	if (IS_ERR(bh))
		return bh;
	err = ext5_wait_block_bitmap(sb, block_group, bh);
	if (err) {
		put_bh(bh);
		return ERR_PTR(err);
	}
...
/**
 * ext5_should_retry_alloc() - check if a block allocation should be retried
 * @sb:			superblock
 * @retries:		number of retry attempts made so far
 *
 * ext5_should_retry_alloc() is called when ENOSPC is returned while
...
int ext5_should_retry_alloc(struct super_block *sb, int *retries)
{
	...
	if (sbi->s_mb_free_pending == 0) {
		...
		return ext5_has_free_clusters(sbi, 1, 0);
	}
	 ...
	 */
	ext5_debug("%s: retrying operation after ENOSPC\n", sb->s_id);
				  ...
				  unsigned long *count, int *errp)
{
	 ...
	 */
	if (!(*errp) && (flags & EXT5_MB_DELALLOC_RESERVED)) {
...
ext5_fsblk_t ext5_count_free_clusters(struct super_block *sb)
{
	...
	for (i = 0; i < ngroups; i++) {
		...
		if (!gdp)
			continue;
		grp = NULL;
		if (EXT5_SB(sb)->s_group_info)
			grp = ext5_get_group_info(sb, i);
		if (!grp || !EXT5_MB_GRP_BBITMAP_CORRUPT(grp))
...
int ext5_bg_has_super(struct super_block *sb, ext5_group_t group)
{
	...
	if ((group <= 1) || !ext5_has_feature_sparse_super(sb))
		return 1;
	if (!(group & 1))
		return 0;
 ...
 */
unsigned int ext5_num_base_meta_blocks(struct super_block *sb,
				       ext5_group_t block_group)
{
	struct ext5_sb_info *sbi = EXT5_SB(sb);
	unsigned num;
	...
	if (!ext5_has_feature_meta_bg(sb) ||
		...
		if (num) {
			num += ext5_bg_num_gdb_nometa(sb, block_group);
			num += le16_to_cpu(sbi->s_es->s_reserved_gdt_blocks);
		}
	} else { /* For META_BG_BLOCK_GROUPS */
		num += ext5_bg_num_gdb_meta(sb, block_group);
	}
	return num;
}

static unsigned int ext5_num_base_meta_clusters(struct super_block *sb,
						ext5_group_t block_group)
{
	return EXT5_NUM_B2C(EXT5_SB(sb), ext5_num_base_meta_blocks(sb, block_group));
}
