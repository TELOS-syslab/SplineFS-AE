...
int ext5_inode_journal_mode(struct inode *inode)
{
	if (EXT5_JOURNAL(inode) == NULL)
		return EXT5_INODE_WRITEBACK_DATA_MODE;	/* writeback */
	/* We do not support data journalling with delayed allocation */
	if (!S_ISREG(inode->i_mode) ||
	    ext5_test_inode_flag(inode, EXT5_INODE_EA_INODE) ||
	    test_opt(inode->i_sb, DATA_FLAGS) == EXT5_MOUNT_JOURNAL_DATA ||
	    (ext5_test_inode_flag(inode, EXT5_INODE_JOURNAL_DATA) &&
	    !test_opt(inode->i_sb, DELALLOC) &&
	    !mapping_large_folio_support(inode->i_mapping))) {
		/* We do not support data journalling for encrypted data */
		if (S_ISREG(inode->i_mode) && IS_ENCRYPTED(inode))
			return EXT5_INODE_ORDERED_DATA_MODE;  /* ordered */
		return EXT5_INODE_JOURNAL_DATA_MODE;	/* journal data */
	}
	if (test_opt(inode->i_sb, DATA_FLAGS) == EXT5_MOUNT_ORDERED_DATA)
		return EXT5_INODE_ORDERED_DATA_MODE;	/* ordered */
	if (test_opt(inode->i_sb, DATA_FLAGS) == EXT5_MOUNT_WRITEBACK_DATA)
		return EXT5_INODE_WRITEBACK_DATA_MODE;	/* writeback */
	BUG();
}
 ...
 */
static int ext5_journal_check_start(struct super_block *sb)
{
	int ret;
	journal_t *journal;
	...
	if (journal && is_journal_aborted(journal)) {
		ext5_abort(sb, -journal->j_errno, "Detected aborted journal");
		return -EROFS;
	}
	return 0;
}

handle_t *__ext5_journal_start_sb(struct inode *inode,
				  ...
				  int revoke_creds)
{
	...
	int err;
	if (inode)
		trace_ext5_journal_start_inode(inode, blocks, rsv_blocks,
					revoke_creds, type,
					_RET_IP_);
	else
		trace_ext5_journal_start_sb(sb, blocks, rsv_blocks,
					revoke_creds, type,
					_RET_IP_);
	err = ext5_journal_check_start(sb);
	if (err < 0)
		return ERR_PTR(err);

	journal = EXT5_SB(sb)->s_journal;
	if (!journal || (EXT5_SB(sb)->s_mount_state & EXT5_FC_REPLAY))
					...
					int type)
{
	...
	trace_ext5_journal_start_reserved(sb,
				  ...
				  int extend_cred, int revoke_cred)
{
	...
	return ext5_journal_extend(handle, extend_cred, revoke_cred);
}

static void ext5_journal_abort_handle(const char *caller, unsigned int line,
				      const char *err_fn,
...
static void ext5_check_bdev_write_error(struct super_block *sb)
{
				    ...
				    enum ext5_journal_trigger_type trigger_type)
{
	...
	if (ext5_handle_valid(handle)) {
		...
		ext5_check_bdev_write_error(sb);
	if (trigger_type == EXT5_JTR_NONE ||
	    !ext5_has_feature_metadata_csum(sb))
		return 0;
	BUG_ON(trigger_type >= EXT5_JOURNAL_TRIGGER_COUNT);
	jbd3_journal_set_triggers(bh,
		&EXT5_SB(sb)->s_journal_triggers[trigger_type].tr_triggers);
	return 0;
}
		  ...
		  struct buffer_head *bh, ext5_fsblk_t blocknr)
{
	...
	ext5_debug("forgetting bh %p: is_metadata=%d, mode %o, data mode %x\n",
	...
	if (err) {
		ext5_journal_abort_handle(where, line, __func__,
					  bh, handle, err);
		__ext5_error(inode->i_sb, where, line, true, -err, 0,
				...
				enum ext5_journal_trigger_type trigger_type)
{
	...
	if (err) {
		ext5_journal_abort_handle(where, line, __func__, bh, handle,
					  err);
		return err;
	}
	if (trigger_type == EXT5_JTR_NONE ||
	...
	if (ext5_handle_valid(handle)) {
		...
		if (!is_handle_aborted(handle) && WARN_ON_ONCE(err)) {
			ext5_journal_abort_handle(where, line, __func__, bh,
						  handle, err);
			if (inode == NULL) {
				pr_err("EXT5: jbd3_journal_dirty_metadata "
		...
		if (inode && inode_needs_sync(inode)) {
			...
			if (buffer_req(bh) && !buffer_uptodate(bh)) {
				ext5_error_inode_err(inode, where, line,
						     bh->b_blocknr, EIO,
